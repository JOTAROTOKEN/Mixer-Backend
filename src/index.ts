...
//mainnets
const chains = [1, 56];
const values = ["0.1","0.5","1","5","10","100"]; // deposit values
...
const relayerAddress = "0xD14e0D9DB23A7925c6C19C28D9A616d873357CBD"; // the wallet to receive the fee
const feePercent = 15; // 0.15%

const startBlock = {
    1: 17067606,
    56: 27431679,
    97: 28310830, // block number of deployment for "0.1" ETHpeer on bsctest
    5: 8708125 // block number of deployment for "0.1" ETHpeer on goerlietest
};
...
app.post('/getProof', async (req, res)=>{
    ...
    try
    {
        let data = await generateProof({
            ...
            fee: ethers.utils.parseEther((Number(parsedData.amount)*feePercent/10000).toString()),
            peer: peerContract
        });
